...
// [ state q'C1: 3 bit(s), variable q'y: 32 bit(s), variable q'C1'started: 1 bit(s), variable q'x: 27 bit(s) ],
// Combined with a non-leaf vector tree node: [ variable q'x: 5 bit(s) ]
...
inline __device__ void get_q_C1(statetype *b, nodetype part1, nodetype part2) {
	uint16_t t2;
	asm("{\n\t"
		" .reg .u64 t1;\n\t"
		" bfe.u64 t1, %1, 60, 3;\n\t"
...
inline __device__ void get_q_y(elem_inttype *b, nodetype part1, nodetype part2) {
	asm("{\n\t"
		" .reg .u64 t1;\n\t"
		" bfe.u64 t1, %1, 28, 32;\n\t"
		" cvt.u32.u64 %0, t1;\n\t"
...
inline __device__ void get_q_C1_started(elem_booltype *b, nodetype part1, nodetype part2) {
	uint16_t t2;
	asm("{\n\t"
		" .reg .u64 t1;\n\t"
		" bfe.u64 t1, %1, 27, 1;\n\t"
...
inline __device__ void get_q_x(elem_inttype *b, nodetype part1, nodetype part2) {
	asm("{\n\t"
		" .reg .u64 t1;\n\t"
		" bfe.u64 t1, %2, 23, 5;\n\t"
		" bfi.b64 t1, %1, t1, 0, 27;\n\t"
		" cvt.u32.u64 %0, t1;\n\t"
	    "}" : "=r"(*b) : "l"(part1), "l"(part2));
}
...
// Precondition: the given parts indeed contain the requested info.
inline void host_get_q_C1(statetype *b, nodetype part1, nodetype part2) {
	nodetype t1 = part1;
...
inline void host_get_q_y(elem_inttype *b, nodetype part1, nodetype part2) {
	nodetype t1 = part1;
	// Strip away data beyond the requested data.
...
inline void host_get_q_C1_started(elem_booltype *b, nodetype part1, nodetype part2) {
	nodetype t1 = part1;
	// Strip away data beyond the requested data.
	t1 = t1 & 0xfffffff;
	// Right shift to isolate requested data.
...
inline void host_get_q_x(elem_inttype *b, nodetype part1, nodetype part2) {
	nodetype t1 = part1;
	nodetype t2 = part2;
	// Strip away data beyond the requested data.
	t2 = t2 & 0xfffffff;
...
inline __device__ void set_left_q_C1(nodetype *part, elem_chartype x) {
...
inline __device__ void set_left_q_y(nodetype *part, elem_inttype x) {
	nodetype t1 = (nodetype) x;
	asm("{\n\t"
		" bfi.b64 %0, %1, %0, 28, 32;\n\t"
		"}" : "+l"(*part) : "l"(t1));
...
inline __device__ void set_left_q_C1_started(nodetype *part, elem_booltype x) {
	nodetype t1 = (nodetype) x;
	asm("{\n\t"
		" bfi.b64 %0, %1, %0, 27, 1;\n\t"
		"}" : "+l"(*part) : "l"(t1));
...
inline __device__ void set_left_q_x(nodetype *part, elem_inttype x) {
...
inline __device__ void set_right_q_x(nodetype *part, elem_inttype x) {
	nodetype t1 = (nodetype) x;
	asm("{\n\t"
		" bfi.b64 %0, %1, %0, 23, 5;\n\t"
		"}" : "+l"(*part) : "l"(t1));
...
// of that state.
inline __device__ uint32_t get_part_bitmask_q_C1(statetype sid) {
	switch (sid) {
		case 0:
			return 0x0;
		case 1:
...
inline __device__ uint32_t get_part_bitmask_for_states_in_vectorpart(uint8_t pid, nodetype part1, nodetype part2) {
	...
	switch (pid) {
		case 0:
			get_q_C1(&s, part1, part2);
			result = result | get_part_bitmask_q_C1(s);
			return result;
		case 1:
			return result;
		default:
...
inline __device__ void explore_q_C1(shared_indextype node_index) {
	...
	switch (current) {
		case 0:
			...
			// Store new values.
			part2 = part1;
			set_left_q_C1(&part2, (statetype) target);
			if (part2 != part1) {
		...
		case 1:
			...
			if (buf32_0 > 0) {
				...
				part2 = part1;
				set_left_q_C1(&part2, (statetype) target);
				set_left_q_x(&part2, buf32_0);
				set_left_q_y(&part2, buf32_1);
				if (part2 != part1) {
					// This part has been altered. Store it in shared memory and remember address of new part.
					part_cachepointers = CACHE_POINTERS_NEW_LEAF;
					buf16_0 = STOREINCACHE(part2, part_cachepointers);
		...
		case 3:
			...
			// SimpleState0 --{ started := True }--> Com1
			...
			part2 = part1;
			set_left_q_C1(&part2, (statetype) target);
			set_left_q_C1_started(&part2, buf1_0);
			if (part2 != part1) {
				// This part has been altered. Store it in shared memory and remember address of new part.
				part_cachepointers = CACHE_POINTERS_NEW_LEAF;
				buf16_0 = STOREINCACHE(part2, part_cachepointers);
...
void print_content_hash_table(FILE* stream, compressed_nodetype *q, nodetype *q_i, uint64_t q_size, uint64_t q_i_size, bool print_pointers) {
	...
	for (uint64_t i = 0; i < q_size; i++) {
		if (q[i] != EMPTY_COMPRESSED_NODE) {
			...
			p1 = &part0;
			p2 = p1;
			host_get_q_C1(&e_st, *p1, *p2);
			fprintf(stream, "state q'C1: %u\n", (uint32_t) e_st);
			p1 = &part0;
			p2 = p1;
			host_get_q_y(&e_i, *p1, *p2);
			fprintf(stream, "variable q'y: %u\n", (uint32_t) e_i);
			p1 = &part0;
			p2 = p1;
			host_get_q_C1_started(&e_b, *p1, *p2);
			fprintf(stream, "variable q'C1'started: %u\n", (uint32_t) e_b);
			p1 = &part0;
			p2 = &part1;
			host_get_q_x(&e_i, *p1, *p2);
			fprintf(stream, "variable q'x: %u\n", (uint32_t) e_i);
			fprintf(stream, "-----\n");
		}
	}
	if (print_pointers) {
