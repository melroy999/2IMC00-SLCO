...
def render_vercors_transition(model: Transition) -> str:
    ...
    for s in model.statements:
        ...
        if isinstance(s, Composite):
            result, i = render_vercors_composite(s, control_node_methods, transition_prefix, i)
        elif isinstance(s, Assignment):
            result, i = render_vercors_assignment(s, control_node_methods, transition_prefix, i)
        elif isinstance(s, (Expression, Primary)):
            result, i = render_vercors_root_expression(s, control_node_methods, transition_prefix, i)
        else:
...
def render_vercors_lock_manager(model: Class) -> str:
...
env.filters["render_vercors_lock_manager"] = render_vercors_lock_manager
