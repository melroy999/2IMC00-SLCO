...
class VercorsModelRenderer(JavaModelRenderer):
    ...
    def __init__(self):
        ...
        self.state_machine_constructor_body_template = self.env.get_template(
            "vercors/base/state_machine_constructor_body.jinja2template"
        )
        self.state_machine_variable_declarations_template = self.env.get_template(
            "vercors/base/state_machine_variable_declarations.jinja2template"
        )
        self.state_machine_template = self.env.get_template("vercors/base/state_machine.jinja2template")
        self.class_template = self.env.get_template("vercors/base/class.jinja2template")
        self.model_template = self.env.get_template("vercors/base/model.jinja2template")

        # Add additional templates.
        self.common_permissions_template = self.env.get_template(
            "vercors/base/util/common_permissions.jinja2template"
        )
        self.base_class_permissions_template = self.env.get_template(
            "vercors/base/util/base_class_permissions.jinja2template"
        )
        self.base_state_machine_permissions_template = self.env.get_template(
            "vercors/base/util/base_state_machine_permissions.jinja2template"
        )
        self.method_contract_template = self.env.get_template(
            "vercors/base/util/method_contract.jinja2template"
        )
        self.state_machine_constructor_contract_template = self.env.get_template(
            "vercors/base/state_machine_constructor_contract.jinja2template"
        )
        self.class_constructor_contract_template = self.env.get_template(
            "vercors/base/class_constructor_contract.jinja2template"
        )
        self.range_check_assumption_method_template = self.env.get_template(
            "vercors/base/range_check_assumption.jinja2template"
        )
    ...
    def render_range_check_body_statements(self, model: SlcoStatementNode) -> str:
        ...
        range_check_statements = list(OrderedDict.fromkeys(range_check_statements))
        return self.join_with_strip([range_check_comment] + range_check_statements)
    ...
    def render_full_state_machine_contract_permissions(self) -> str:
        ...
        return self.join_with_strip([permission_comment] + permission_statements)
    ...
    def render_result_contract_statement(self, model: SlcoStatementNode, use_old_values: bool = False):
        ...
        return self.join_with_strip([result_check_comment, result_check_statement])
    ...
    def get_assignment_opening_body(self, model: Assignment) -> str:
        ...
        return self.join_with_strip([result, range_check_assumption_method_name])
    ...
    def render_method_contract(self, target_statements: List[str], pure_functions: List[str]) -> str:
        ...
        contract_body = self.join_with_strip(target_statements, join_str="\n\n")
