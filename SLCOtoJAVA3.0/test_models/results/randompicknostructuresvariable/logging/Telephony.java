package testing.randompicknostructuresvariable.logging;
...
public class Telephony {
    ...
    static {
        ...
        String log_settings = "[LA,NDS,T=30s,URP]";
    ...
    private static class GlobalClass implements SLCO_Class {
        ...
        class GlobalClass_User_0Thread extends Thread implements GlobalClass_User_0Thread_States {
            ...
            private boolean t_idle_0_s_0_n_0() {
                lock_ids[0] = target_locks[0] = 8 + 0; // Acquire chan[0]
            ...
            private boolean execute_transition_idle_1() {
                ...
                lock_ids[0] = target_locks[1] = 12 + 0; // Acquire partner[0]
                lockManager.acquire_locks(lock_ids, 1);
            ...
            private boolean t_calling_0_s_0_n_0() {
                lock_ids[0] = target_locks[0] = 12 + 0; // Acquire partner[0]
                lockManager.acquire_locks(lock_ids, 1);
                if(partner[0] == 0) {
            ...
            private boolean t_calling_3_s_0_n_0() {
                lock_ids[0] = target_locks[3] = 0 + 0; // Acquire callforwardbusy[0]
                lockManager.acquire_locks(lock_ids, 1);
                lock_ids[0] = target_locks[2] = 4 + 0; // Acquire record[0]
                lockManager.acquire_locks(lock_ids, 1);
                lock_ids[0] = target_locks[1] = 8 + 0; // Acquire chan[0]
                lockManager.acquire_locks(lock_ids, 1);
                lock_ids[0] = target_locks[0] = 12 + 0; // Acquire partner[0]
                lockManager.acquire_locks(lock_ids, 1);
                if(partner[0] != 0) {
                ...
                lock_ids[2] = target_locks[2]; // Release record[0]
                lock_ids[3] = target_locks[3]; // Release callforwardbusy[0]
            ...
            private boolean t_calling_3_s_0_n_3() {
                if(callforwardbusy[partner[0]] == 255) {
                    lock_ids[0] = target_locks[3]; // Release callforwardbusy[0]
                    lockManager.release_locks(lock_ids, 1);
                    return true;
                }
                lock_ids[0] = target_locks[0]; // Release partner[0]
                lock_ids[1] = target_locks[2]; // Release record[0]
                lock_ids[2] = target_locks[3]; // Release callforwardbusy[0]
            ...
            private boolean execute_transition_calling_4() {
                ...
                // SLCO assignment | record[partner[0]] := 0.
                record[partner[0]] = (0) & 0xff;
                lock_ids[0] = target_locks[2]; // Release record[0]
                lockManager.release_locks(lock_ids, 1);
                // SLCO assignment | partner[0] := callforwardbusy[partner[0]].
                partner[0] = (callforwardbusy[partner[0]]) & 0xff;
                lock_ids[0] = target_locks[0]; // Release partner[0]
                lock_ids[1] = target_locks[3]; // Release callforwardbusy[0]
                lockManager.release_locks(lock_ids, 2);
            ...
            private boolean execute_transition_ringback_0() {
                ...
                // (Superfluous) SLCO expression | true.
                // SLCO assignment | chan[0] := 255.
                lock_ids[0] = target_locks[2] = 8 + 0; // Acquire chan[0]
                lockManager.acquire_locks(lock_ids, 1);
                chan[0] = (255) & 0xff;
